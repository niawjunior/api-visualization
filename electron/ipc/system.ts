...
export function registerSystemHandlers() {
    ipcMain.handle('open-path', async (event, targetPath: string, line?: number, appName?: 'antigravity' | 'vscode' | 'cursor' | 'system') => {
        ...
        if (appName && appName !== 'system' && process.platform === 'darwin') {
             ...
             spawn('open', ['-a', nameToOpen, '--args', targetPath]); // Most apps don't support standardized line numbers via generic open
             return { success: true };
        }
            ...
            for (const editorPath of paths) {
                for (const known of KNOWN_EDITORS) {
                    ...
                    if (editorPath.toLowerCase().includes(known.name.toLowerCase().replace(' ', ''))) {
                         ...
                         let key = known.name;
                         if (known.id === 'com.microsoft.VSCode') key = 'vscode';
                         if (known.id === 'com.todesktop.230313mzl4w4u92') key = 'cursor';
                         if (known.id === 'com.google.antigravity') key = 'antigravity';

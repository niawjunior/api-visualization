...
import type { ResponseSchema, ExtractionContext, PropertySchema } from '../types';
import { resolveExpressionType } from '../utils/type-utils';
...
function extractResponseFromReturn(
    ...
    if (expr.expression.name.text !== 'json') return null;
    
    const args = expr.arguments;
    if (args.length === 0) return null;
    
    const responseArg = args[0];
    const { properties } = resolveExpressionType(ctx.checker, responseArg, ctx.variables);
...
function extractStatusFromOptions(options: ts.ObjectLiteralExpression): { statusCode?: number; isError: boolean } {
...
function generateSignatureKey(response: ResponseSchema): string {
...
export function detectZodSchema(
    ...
    if (methodName !== 'parse' && methodName !== 'safeParse') return null;
